...
**Префикс аннотаций сменился, и это укладывает сайты на пять дней.** В external-dns v0.22.0 аннотации вышли из alpha ([kubernetes-sigs/external-dns#6424](https://github.com/kubernetes-sigs/external-dns/pull/6424)): префикс по умолчанию теперь `external-dns.kubernetes.io/`, а старый `external-dns.alpha.kubernetes.io/` не читается вообще, без фолбэка. Release notes предупреждают об удалении записей, и на всякий случай `--policy` сделали обязательным флагом ([#6508](https://github.com/kubernetes-sigs/external-dns/pull/6508)). У меня сценарий оказался хуже удаления, потому что тише. Renovate поднял чарт, под перезапустился, и в первую же минуту в логе прошла пачка `Changing record. action=UPDATE` по всем публичным именам. Дальше тишина: ArgoCD зелёный, под Running, ошибок ноль. А сайты не открываются.

Механика в два хода. `--annotation-filter` матчит по наличию ключа в сырых аннотациях, поэтому маршруты со старым `cloudflare-proxied` фильтр прошли и записи никуда не делись. Но сам флаг `proxied` провайдер читает уже из `external-dns.kubernetes.io/cloudflare-proxied`, которого на маршрутах нет, и при `policy: sync` он аккуратно проставил `proxied=false` на всё. Это первая грабля из этого списка, только без единой ошибки. Вторая ловушка живёт в DNSEndpoint: `providerSpecific` со старым именем ключа external-dns считает свойством, которого у текущей записи нет, и обновляет запись в Cloudflare API каждую минуту, бесконечно.

Порядок миграции важен именно из-за фильтра. Сначала добавить GA-ключ рядом со старым на каждом ресурсе, а `providerSpecific` в DNSEndpoint переименовать сразу, дубль оставит флаппинг. Дождаться выкатки. Потом перевести `--annotation-filter` на новый ключ. И только потом снести alpha-ключи. Перевернуть фильтр первым нельзя: ноль совпадений, и `--policy=sync` удалит всё, как в release notes и написано. Есть и путь без переписывания манифестов: `--annotation-prefix=external-dns.alpha.kubernetes.io/` заставит external-dns читать старый префикс как основной. Этот флаг я сам приносил в external-dns под другую задачу ([#5889](https://github.com/kubernetes-sigs/external-dns/pull/5889), про неё во второй части), и не ожидал, что он же станет дорогой миграции для всех.
